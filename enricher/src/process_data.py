...
def load_and_filter_data(base_df, path, col_to_filter="name", name_map=None):
    ...
    filtered_org_data = org_df[
        org_df[col_to_filter].isin(base_df["company_name"])
...
def load_combined_funding_data(base_df):
    ...
    funding_data = load_and_filter_data(
        base_df,
        "data/interview-test-funding.json.gz",
        "org_name",
        name_map=name_map_funding,
    )
    funding_data_2019 = load_and_filter_data(
        base_df,
        "data/interview-test-funding-2019.json.gz",
        "company_name",
        name_map=name_map_funding,
    )
    funding_data_merged = pd.merge(
        funding_data,
        funding_data_2019,
        how="outer",
        on="funding_round_uuid",
        suffixes=["__base", "__2019"],
    )
    for col in cols_to_coalesce:
        funding_data_merged[col] = funding_data_merged[col + "__base"].combine_first(
            funding_data_merged[col + "__2019"]
        )

    # Clean up the messy investors data field
    funding_data_merged["investors"] = (
        funding_data_merged["investor_names"]
...
def load_combined_org_data(base_df):
    ...
    org_data = load_and_filter_data(
        base_df,
        "data/interview-test-org.json.gz",
        "name",
        name_map=name_map_orgs,
    )
    org_data_2019 = load_and_filter_data(
        base_df,
        "data/interview-test-org-2019.json.gz",
        "company_name",
        name_map=name_map_orgs,
    )
    org_data_merged = pd.merge(
        org_data,
        org_data_2019,
        how="outer",
        on="company_uuid",
    ...
    for col in cols_to_coalesce:
        org_data_merged[col] = org_data_merged[col + "__base"].combine_first(
            org_data_merged[col + "__2019"]
        )
...
def create_combined_dataset(base_df, funding_df, org_df):
    ...
    agg_funding_data = funding_data_grouped.agg(
        {
            "announced_on": ["min", "max"],
            "investor_count": "sum",
            "raised_amount_usd": "sum",
            "investors": list_agg,
            "investment_type": list_agg,
        }
    )
    agg_funding_data.columns = [
        "_".join(a) for a in agg_funding_data.columns.to_flat_index()
    ]
    ...
    org_data_filtered = (
        org_df.sort_values(["company_name", "last_funding_on"], ascending=False)
        .groupby("company_name")
        .first()
    )
    org_data_filtered = org_data_filtered.rename(org_data_rename_map, axis=1)[
        list(org_data_rename_map.values())
    ]
    return base_df.join(agg_funding_data, on="company_name", how="left").join(
        org_data_filtered, on="company_name", how="left"
    )


def write_output(df):
    output_file = "enriched_data.json.gz"
    output_dir = Path("output")
    output_dir.mkdir(parents=True, exist_ok=True)
    df.to_json(output_dir / output_file, orient="records", lines=True)
